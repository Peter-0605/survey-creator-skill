...
`survey-creator-skill` is an open-source skill and toolchain for building production-safe survey experiences from structured schema. It does more than render HTML: it validates schema legality, repairs safe issues, renders submittable survey pages, verifies runtime behavior in browser E2E, checks interaction flow, validates accessibility, and guarantees that submitted payloads conform to the exact survey schema. It is especially useful for AI-assisted survey generation, AI Native form workflows, and teams that want stronger guardrails before shipping questionnaires to real users.
...
`survey-creator-skill` 是一个开源的 skill + 工具链，用来把结构化问卷 schema 生成成可提交的 HTML 问卷页面，并在交付前完成整套合法性校验。它不只是“生成页面”，还包括：schema 校验、安全修复、HTML 渲染、浏览器运行时检查、交互 E2E、可访问性验证、payload 合法性校验，以及“提交数据必须与具体问卷 schema 完全一致”的最终兜底。它尤其适合 AI 辅助生成问卷、AI Native 表单流程，以及任何希望在真实投放前把风险拦在上线前的团队。
